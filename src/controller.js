...
quizQuestionsView.onTimeUp = () => {
  if (model.isQuizOver()) {
    quizQuestionsView.stopTimer();
    controlResult();
  } else {
    controlTimeUp();
  }
...
const controlQuestion = function () {
  ...
  quizQuestionsView.renderCountdown(countdownTime);
...
function controlTimeUp() {
  const question = model.nextQuestion();
  const index = model.state.currentQuestionIndex;
  const quiz = model.state.currentQuiz;
